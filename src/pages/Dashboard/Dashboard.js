...
export function renderDashboard() {
  return `<div class="dashboard"><div id="dashboard-summary-message" class="dashboard-summary-message"></div><div class="dashboard-summary-cards">${renderSummaryCard('TOTAL ORDER', 'totalOrder', '')}${renderSummaryCard('ON ORDER', 'onOrder', 'ON ORDER')}${renderSummaryCard('PART ARRIVAL', 'partArrival', 'PART ARRIVAL')}${renderSummaryCard('BOOKING', 'booking', 'BOOKING')}${renderSummaryCard('NO SHOW', 'noShow', 'NO SHOW')}</div><section class="dashboard-todo-section"><div class="dashboard-todo-header"><div><h3>TO DO LIST TODAY</h3></div><span id="dashboard-todo-date" class="dashboard-todo-date"></span></div><div id="dashboard-todo-message" class="dashboard-todo-message">Memuat To Do List Today...</div><div id="dashboard-todo-list" class="dashboard-todo-list"></div></section><section class="dashboard-sa-section"><div class="dashboard-sa-header"><h3>MONITORING SA</h3><label class="dashboard-sa-selector"><span>Pilih SA</span><select id="dashboard-sa-select">${SA_LIST.map(sa => `<option value="${sa}" ${sa === selectedSA ? 'selected' : ''}>${sa}</option>`).join('')}</select></label></div><div id="dashboard-sa-loading" class="dashboard-sa-message">Memuat monitoring SA...</div><div id="dashboard-sa-content" class="dashboard-sa-content"></div></section></div>`
}

function renderSummaryCard(label, key, status) { return `<button type="button" class="dashboard-summary-card dashboard-summary-card-clickable ${statusCardClass(status)}" data-dashboard-status="${status}" aria-label="Buka All Order ${label}"><span class="dashboard-summary-label">${label}</span><strong id="dashboard-summary-${key}" class="dashboard-summary-value">-</strong></button>` }
function statusCardClass(status) { if (!status) return 'dashboard-summary-total'; return `dashboard-summary-${String(status).toLowerCase().replaceAll(' ', '-')}` }
...
function initSummaryCardNavigation() { document.querySelectorAll('.dashboard-summary-card').forEach(card => card.addEventListener('click', () => document.dispatchEvent(new CustomEvent('open-all-order-filter', { detail: { statusFilter: card.dataset.dashboardStatus || '' } }))) }
function initSASelector() { document.getElementById('dashboard-sa-select')?.addEventListener('change', event => { selectedSA = event.target.value || SA_LIST[0]; loadSAMonitoring() }) }
...
function renderSAMonitoring(summary, notifications) {
  ...
  content.innerHTML = `<div class="dashboard-sa-grid"><button type="button" class="dashboard-sa-order-card" data-open-sa-order="${selectedSA}"><div class="dashboard-sa-card-title"><strong>ORDER ${selectedSA}</strong><span>${summary.total} WO</span></div><div class="dashboard-sa-chart-wrap"><div class="dashboard-sa-donut" style="background:${donutBackground}"><div><strong>${summary.total}</strong><span>TOTAL WO</span></div></div><div class="dashboard-sa-legend">${parts.map(item => `<span><i class="dashboard-sa-dot ${item.cls}"></i>${item.label}<strong>${item.value}</strong></span>`).join('')}</div></div></button><div class="dashboard-sa-notification-card"><div class="dashboard-sa-card-title"><strong>NOTIFICATION ${selectedSA}</strong><span>${notificationsTotal} Active</span></div><div class="dashboard-sa-notification-list">${renderSAGroupedNotifications(notifications)}</div></div></div>`

  document.querySelector('[data-open-sa-order]')?.addEventListener('click', () => document.dispatchEvent(new CustomEvent('open-monitoring-sa', { detail: { sa: selectedSA } })))
  document.querySelectorAll('[data-open-sa-notification]').forEach(item => item.addEventListener('click', () => document.dispatchEvent(new CustomEvent('open-notification-center', { detail: { type: item.dataset.type, sa: selectedSA, todayOnly: false } }))))
}
...
function renderSAGroupedNotifications(notifications) {
  ...
  return active.map(type => { const definition = NOTIFICATION_DEFINITIONS[type]; return `<button type="button" class="dashboard-sa-notification-item" data-open-sa-notification data-type="${type}"><span class="dashboard-sa-notification-icon">${definition.icon}</span><strong>${definition.title}</strong><span>${notifications[type].length}</span></button>` }).join('')
}
...
function renderTodayTodos(todos) {
  ...
  if (!todos.length) { message.textContent = 'Tidak ada To Do baru hari ini.'; list.innerHTML = '<div class="dashboard-todo-empty">Semua kondisi hari ini sudah bersih.</div>'; return }
  message.textContent = ''
  ...
  document.querySelectorAll('[data-todo-type]').forEach(card => card.addEventListener('click', () => document.dispatchEvent(new CustomEvent('open-notification-center', { detail: { type: card.dataset.todoType, todayOnly: true } }))))
}
