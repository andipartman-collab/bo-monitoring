...
export async function initNotificationCenter(params = {}) {
  ...
  selectedSAFilter = String(params.sa || '').trim().toUpperCase()
  notificationData = null
  bindGroupTabs()
  syncGroupTabState()

  try {
    const allNotifications = await buildNotifications()
    notificationData = filterNotificationData(allNotifications, selectedSAFilter)

    if (params.todayOnly && selectedType) {
      const todayData = await buildTodayTodoNotifications()
      const filteredTodayData = filterNotificationData(todayData, selectedSAFilter)
      selectedRowsOverride = filteredTodayData[selectedType] || []
    }
...
function filterNotificationData(data, sa) {
  const filtered = Object.fromEntries(Object.keys(NOTIFICATION_DEFINITIONS).map(type => [type, []]))
  if (!sa) return data
  for (const type of Object.keys(filtered)) {
    filtered[type] = (data[type] || []).filter(row => String(row.sa || '').trim().toUpperCase() === sa)
  }
  return filtered
}
...
function renderSummary() {
  ...
  element.innerHTML = `<div class="notification-summary-card total"><span>TOTAL NOTIFICATION${selectedSAFilter ? ` — ${selectedSAFilter}` : ''}</span><strong>${totalWO + totalPART}</strong></div><div class="notification-summary-card wo"><span>WO</span><strong>${totalWO}</strong></div><div class="notification-summary-card part"><span>PART</span><strong>${totalPART}</strong></div>`
}
...
function bindWOButtons() { document.querySelectorAll('[data-order-id]').forEach(button => button.addEventListener('click', () => document.dispatchEvent(new CustomEvent('open-notification-wo-detail', { detail: { orderId: button.dataset.orderId } })))) }
function renderSummarySkeleton() { return `<div class="notification-summary-card"><span>TOTAL NOTIFICATION</span><strong>—</strong></div><div class="notification-summary-card"><span>WO</span><strong>—</strong></div><div class="notification-summary-card"><span>PART</span><strong>—</strong></div>` }
